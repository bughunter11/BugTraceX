...
def update_tool():
    ...
    version_file = os.path.expanduser("~/version.txt")  # ✅ Home directory path

    # === Step 1: Read local version ===
    try:
        with open(version_file, "r") as vf:
            local_version = vf.read().strip()
            if not local_version or not local_version.replace(".", "").isdigit():
                raise ValueError
    except:
        local_version = "0.0"  # If file missing or invalid

    # === Step 2: GitHub version check ===
    timestamp = int(time.time())  # ⏱️ Force fresh request
    version_url = f"https://raw.githubusercontent.com/bughunter11/BugTraceX/main/version.txt?t={timestamp}"
    script_url = f"https://raw.githubusercontent.com/bughunter11/BugTraceX/main/BugTraceX.py?t={timestamp}"
    ...
    try:
        ...
        remote_version = requests.get(version_url, headers=headers).text.strip()
    except Exception as e:
    ...
    print(CYAN + f"[~] Local Version : {local_version}")
    print(CYAN + f"[~] Remote Version: {remote_version}")

    if local_version.lstrip("v") == remote_version.lstrip("v"):
        ...
        if choice != "y":
            print(RED + "[×] Update cancelled.")
            time.sleep(2)
            return

    # === Step 4: Download new script ===
    print(YELLOW + "[*] Downloading new version from GitHub...")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(GREEN + f"\n[✓] Updated to version {remote_version} successfully!")
            print(GREEN + "[→] Please restart Termux and run the tool again.\n")
        else:
            print(RED + f"[✗] GitHub error: Status code {response.status_code}")
    except Exception as e:
        print(RED + f"[!] Update failed: {str(e)}")
